...
# Emit .pr-test.local.yml with inline credentials (gitignored — see .gitignore).
{
  ...
  for role in "${!ROLE_EMAILS[@]}"; do
    echo "    - name: $role"
    echo "      email: ${ROLE_EMAILS[$role]}"
    echo "      password: $PASSWORD"
    echo "      totp_seed: ${ROLE_SEEDS[$role]}"
  done
} > .pr-test.local.yml

echo
echo "✓ Wrote .pr-test.local.yml ($(wc -l < .pr-test.local.yml) lines, $(wc -c < .pr-test.local.yml) bytes)"
echo "✓ ${#ROLE_EMAILS[@]} local AI-tester accounts seeded."
echo
