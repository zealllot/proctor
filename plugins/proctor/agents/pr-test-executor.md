...
description: Execute a single PRoctor lint-only or bash test item and return a structured pass/fail result. Invoked once per non-chrome item by the executing-pr-tests skill. Chrome-devtools items are handled inline by the skill (single shared chrome session — see v0.7.1 contract below). Should not push, comment, or modify the test plan.
...
### 0. Scope (v0.7.1+) — non-chrome items only

This subagent handles **lint-only** and **bash** items only. **Chrome-devtools items are handled inline by the executing-pr-tests skill in the host session**, not by this subagent. The chrome-devtools-mcp server uses a single shared chrome profile; spawning concurrent subagent sessions that all try to drive chrome causes "browser is already running" lock contention (v0.7.0 e2e failure mode, PR #1126 run `pr1126-75eea89-353a49f0`: chrome-MCP threw "Use --isolated to run multiple browser instances" and the executor had to kill all chromes + restart, wasting ~2 minutes). If you receive a chrome-devtools item, return immediately with `{"id": "<id>", "status": "skipped", "reason": "chrome-handled-by-skill", "evidence": "v0.7.1 contract: chrome-devtools items run in the executing-pr-tests skill, not in this subagent"}`.
